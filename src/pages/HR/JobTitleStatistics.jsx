...
const JobTitleStatistics = () => {
  ...
  useEffect(() => {
    fetchJobTitleStats(currentPageParams, debouncedCountryFilter);
  ...
  const fetchJobTitleStats = async (page = 1, country = '') => {
    setIsLoading(true);
    try {
      const data = await hrService.getJobTitleStats({ page, country });
      setStatsData(data.stats || []);
      setPagination(data.pagination || null);
    } catch (err) {
      console.error('Failed to fetch job title stats');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="stats-page">
      <div className="section-header">
        <div className="section-info">
          <h2 className="section-title">Job Title Statistics</h2>
          <p className="section-subtitle">View salary averages separated by job title</p>
        </div>
          ...
          {isLoading ? (
            ...
            <>
              <table className="portal-table">
                ...
                <tbody>
                  {statsData.map((countryStat, index) => (
                    <React.Fragment key={index}>
                      <tr className="group-header" style={{ backgroundColor: 'rgba(99, 102, 241, 0.1)' }}>
                        <td colSpan="5" style={{ fontWeight: '600', color: '#818cf8', padding: '1rem' }}>
                          {countryStat.country || 'Unspecified Country'}
                        </td>
                      </tr>
                      {countryStat.job_titles && countryStat.job_titles.map((job, i) => (
                        <tr key={`${index}-${i}`}>
                          <td style={{ paddingLeft: '2.5rem' }}>{job.job_title}</td>
                          <td>{job.employee_count}</td>
                          <td>₹{parseFloat(job.min_salary).toLocaleString()}</td>
                          <td>₹{parseFloat(job.max_salary).toLocaleString()}</td>
                          <td>₹{parseFloat(job.average_salary).toLocaleString()}</td>
                        </tr>
                      ))}
                    </React.Fragment>
                  ))}
...
export default JobTitleStatistics;
